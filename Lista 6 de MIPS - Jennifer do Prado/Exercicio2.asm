.data
 ...
 Nmr_Carac_Arq: .asciiz "\n\ni) Numero de Caracteres do Arquivo(com os caracteres CR, carriage return e LF, line feed inclusos): " 
 ...
 Leitura:
 	...
 	move $a0, $a1  		#recebe o caractere lido
    	...
    	li $s1, -35     	#13(ASCII do CR, carriage return) - 48 = -35
    	li $s4, -38     	#10(ASCII do LF, line feed) - 48 = -38
    	Numero_Caracteres:
		addi $t7, $t7, 1 #incremento o numero de caracteres no arquivo
	beq $t1, $s1, Leitura	 #se o caractere lido for o codigo de CR, temos que ler o proximo e verificar se este proximo caractere � o codigo de LF
    	bne $t1, $s4, Multiplica #se o valor lido nao for o espaco va para Multiplica
    	beq $t1, $s4, Divide     #se for o espaco precisamos dividir o numero por 10, pois multiplicamos devido ao ultimo numero
    	Maior_valor:
		...
		Qtd_Numero_Arq: 
			...
			j Leitura
 	Saida:
 		j Divide   		 #vai fazer a verifica�ao de maior, menor, produto, soma... com o ultimo valor
 		Retorna:
 			jr $ra 		 # Retorna para a main
 ...
 Arquivo_Para_Vetor:
 	...
 	mul $s1, $s6, 4         	#guardando em $s1 N*4 bytes
        ...
        li $v0, 9               	#Codigo SysCall de aloca��o dinamica
 	...
 	beqz $v0, Saida_Vet 		#if(ch != EOF) goto Saida_Vet 
    	...
    	li $s1, -35     		#13(ASCII do CR, carriage return) - 48 = -35
    	li $s4, -38     		#10(ASCII do LF, line feed) - 48 = -38
    	beq $t8, $s1, Leitura_Arq_Para_Vet #se o caractere lido for o codigo de CR, temos que ler o proximo e verificar se este proximo caractere � o codigo de LF
    	beq $t8, $s4, Divide_Num 	#se o valor lido for o espaco va para Divide_Num
 ...
 Ordena_Vet_Crescente:
        ...
        Percorre_Elemento1:             #� a primeira estrutura de repeticao for, funciona como: for(j = N-1 ; j>=1 ; j--)
            ...
            Percorre_Elemento2:         #� a segunda estrutura de repeticao for, funciona como: for(i = 0 ; i<j ; i++)
                ...
                Proximo_Laco:
                    ...
                    blt	$s5, $s4, Percorre_Elemento2	#se $s5 < $s4, ou seja, se i < j, ent�o va para Percorre_Elemento2
    ...
    Ordena_Vet_Decrescente:
        ...
        subi $s5, $s6, 1     #passando o tamanho N-1 do vetor para registrador $s5, ser� o indice j(ultimo indice do vetor)
        mul $s2, $s5, 4      #(N-1)*4
